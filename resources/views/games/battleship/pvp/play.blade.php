...
<div class="absolute bottom-4 right-4">
  ...
  <input id="volume-slider" type="range" min="0" max="100" step="1" value="{{ (int) round(session('battleship_bg_volume', 0.3) * 100) }}" class="h-1 w-24">
</div>
    ...
    <div class="flex gap-[41px]">
      ...
      <div>
        ...
        <div id="opponent-board" class="grid grid-rows-10 grid-cols-10 gap-[0.06rem]">
          @for($y = 0; $y < 10; $y++)
            @for($x = 0; $x < 10; $x++)
              <div class="w-10 h-10 bg-gray-700 hover:bg-gray-600 cursor-pointer cell-clickable" data-x="{{ $x }}" data-y="{{ $y }}"></div>
            @endfor
          @endfor
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  channel.bind('MoveMade', data => {
    const [x, y] = data.coordinates;
    const targetEl = data.shooter === shooter ? document.querySelector(`#opponent-board [data-x='${x}'][data-y='${y}']`) : document.querySelector(`#player-board [data-x='${x}'][data-y='${y}']`);

    if (data.result === 'agua') {
      waterSound.currentTime = 0; waterSound.play();
      targetEl.classList.replace('bg-gray-700', 'bg-blue-400');
    } else if (data.result === 'tocado') {
      impactSound.currentTime = 0; impactSound.play();
      targetEl.classList.replace('bg-gray-700', 'bg-red-600');
    } else if (data.result === 'hundido') {
      hundidoSound.currentTime = 0; hundidoSound.play();
      data.cells.forEach(([sx, sy]) => {
        const c = document.querySelector(`[data-x='${sx}'][data-y='${sy}']`);
        c.classList.replace('bg-gray-700', 'bg-green-500');
      });
    }

    if (data.gameOver) {
      statusEl.textContent = data.winner === shooter ? '🎉 ¡Has ganado!' : '💀 Has perdido';
    } else {
